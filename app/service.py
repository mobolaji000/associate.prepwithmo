...
class StripeInstance():
    ...
    def payInFull(self,stripe_info):
        pass
    ...
    def markCustomerAsChargedOutsideofStripe(self, stripe_info):
        ...
        transaction = stripe.Invoice.create(
            customer=stripe_info['stripe_customer_id'],
            metadata={'transaction_code': stripe_info['transaction_code']},
        )
    ...
    def chargeCustomerViaACH(self, stripe_info, bank_account_token,chosen_mode_of_payment):
        ...
        if chosen_mode_of_payment == 'installment-payment-ach':
            for k in range(1, int(stripe_info['installment_counter'])):
                date = date = datetime.datetime.fromtimestamp(stripe_info['date_' + str(k)])
                ...
                AppDBUtil.createOrModifyInstallmentInvoice(first_name=stripe_info['name'].split()[0], last_name=stripe_info['name'].split()[1],
                                                           phone_number=stripe_info['phone_number'], email=stripe_info['email'],
                                                           transaction_code=stripe_info['transaction_code'], stripe_customer_id=stripe_info['stripe_customer_id'],
                                                           installment_payment_date=date, installment_payment_amount=amount, stripe_invoice_id=stripe_invoice_object['id'])
        else:
            transaction_total = int(stripe_info['transaction_total'])
...
class SendMessagesToClients():
    ...
    def sendSMS(cls,message='perfectscoremo',from_number='+19564771274',to_number='9725847364',type=''):
        ...
        if type == 'create':
            ...
            created_or_modified_span = "Dear parent,\n\nThis is an automated reminder that your transaction is due. Here are the payment instructions/options (also sent to your email address):"


        # + """1. Go to pay.perfectscoremo.com/input_transaction_code\n\n""" \
        if type != 'to_mo':
